...
void RE2::Init(absl::string_view pattern, const Options& options) {
  ...
  prog_ = suffix_regexp_->CompileToProg(max_mem_*2/3,
                                        options_.minimize_max_mem()
                                            ? ReserveNoDFABudget
                                            : ReserveMaxDFABudget);
  if (prog_ == NULL) {
    if (options_.log_errors())
  ...
  if (options_.minimize_max_mem()) {
    max_mem_ = prog_->MinimumMemoryBudget()*3/2;
  }
...
re2::Prog* RE2::ReverseProg() const {
  absl::call_once(rprog_once_, [](const RE2* re) {
    re->rprog_ =
        re->suffix_regexp_->CompileToReverseProg(re->max_mem_ / 3);
    if (re->rprog_ == NULL) {
