...
class Compiler : public Regexp::Walker<Frag> {
 public:
  ...
  void Setup(Regexp::ParseFlags flags, int64_t max_mem,
             bool minimize_mem_budget, RE2::Anchor anchor);
...
void Compiler::Setup(Regexp::ParseFlags flags, int64_t max_mem,
  ...
  max_mem_ = max_mem;
  minimize_mem_budget_ = minimize_mem_budget;
  if (max_mem <= 0) {
...
Prog* Compiler::Compile(Regexp* re, bool reversed, int64_t max_mem,
                        bool minimize_mem_budget) {
  Compiler c;
  c.Setup(re->parse_flags(), max_mem, minimize_mem_budget,
...
Prog* Compiler::CompileSet(Regexp* re, RE2::Anchor anchor, int64_t max_mem) {
  Compiler c;
  c.Setup(re->parse_flags(), max_mem, false /* minimize_mem_budget */, anchor);
